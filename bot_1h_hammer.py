"""
OKX 무기한 선물(perpetual swap) 1시간봉 캔들 신호 감시 봇
"""
...
LOOKBACK = 15
CLOSE_BUFFER_SEC = 12
STATE_FILE = os.environ.get("STATE_FILE", "alert_state_1h.json")
...
log = logging.getLogger("okx_candle_alert_1h")
...
def run_loop():
    ...
    state = load_state()
    log.info("1시간봉 감시를 시작합니다. (%d개 심볼)", len(symbols))

    while True:
        wait_sec = seconds_until_next_close()
        log.info("다음 1시간봉 마감까지 %.0f초 대기", wait_sec)
        time.sleep(wait_sec)
...
def main():
    parser = argparse.ArgumentParser(description="OKX 1시간봉 캔들 신호 감시 봇")

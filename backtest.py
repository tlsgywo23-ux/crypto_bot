...
- 감시할 타임프레임(TIMEFRAMES)은 실전 봇(bot.py)이 쓰는 값이라 여기서는 가져오지
  ...
  → 백테스트에서 2h/12h/1d처럼 실전과 다른 타임프레임의 승률만 확인해보고
  싶을 때, bot.py(실전 봇)는 전혀 건드리지 않고 이 파일에서만 추가/제거하면 됩니다.
  현재 15m/1h/2h/4h/12h/1d 전부 확인합니다.
...
# 실전 봇(bot.py)의 TIMEFRAMES와 별개로 관리되므로, 여기서 추가/제거해도
# 실전 감시 대상에는 영향이 없습니다.
BACKTEST_TIMEFRAMES = [
    {"tf": "15m", "ms": 15 * 60 * 1000, "lookback": 30},
    {"tf": "1h", "ms": 60 * 60 * 1000, "lookback": 30},
    {"tf": "2h", "ms": 2 * 60 * 60 * 1000, "lookback": 30},
